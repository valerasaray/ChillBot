...
При первом взаимодействии создаётся пользовательский аккаунт с флагом is_admin, значение которого определяется сопоставлением ID с белым списком из env. Проверка на админа выполняется в момент отправки команды модерации.
